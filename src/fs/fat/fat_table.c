    ...
    if (pcur)
    {
        ...
        if (pcur->dirty)
        {
            if (fs->disk.write)
            {
                ...
                if (!fs->disk.write(fs->disk.data, pcur->address, pcur->sector, sectors))
                    return 0;
            }
...
//-----------------------------------------------------------------------------
static struct fat_buffer *fatfs_fat_read_sector(struct fatfs *fs, uint32_t sector)
{
    ...
    // Read next sector
    if (!fs->disk.read(fs->disk.data, pcur->address, pcur->sector, FAT_BUFFER_SECTORS))
    {
...
void fatfs_set_fs_info_next_free_cluster(struct fatfs *fs, uint32_t newValue)
{
    ...
    else
    {
        ...
        if (fs->disk.write)
            fs->disk.write(fs->disk.data, pbuf->address, pbuf->sector, 1);
...
//-----------------------------------------------------------------------------
uint32_t fatfs_count_free_clusters(struct fatfs *fs)
{
    uint32_t i,j;
    uint32_t count = 0;
    struct fat_buffer *pbuf;
