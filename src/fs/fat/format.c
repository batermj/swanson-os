...
static uint8_t
fatfs_calc_cluster_size(uint32_t sectors,
                        int is_fat32) {
...
static int
fatfs_erase_sectors(struct fatfs *fs,
                    uint32_t lba,
                    int count) {
	...
	for (i = 0;i < count; i++) {
		if (!fs->disk.write(fs->disk.data, lba + i, fs->currentsector.sector, 1))
			return 0;
	}
...
static int
fatfs_create_boot_sector(struct fatfs *fs,
	...
	if (!is_fat32) {
		...
		fs->currentsector.sector[22] = (uint8_t)((fs->fat_sectors >> 0) & 0xFF);
		fs->currentsector.sector[23] = (uint8_t)((fs->fat_sectors >> 8) & 0xFF);
		...
		fs->currentsector.sector[32] = (uint8_t)((vol_sectors >> 0x00) & 0xFF);
		fs->currentsector.sector[33] = (uint8_t)((vol_sectors >> 0x08) & 0xFF);
		fs->currentsector.sector[34] = (uint8_t)((vol_sectors >> 0x10) & 0xFF);
		fs->currentsector.sector[35] = (uint8_t)((vol_sectors >> 0x18) & 0xFF);
		...
		// BPB_FATSz32
		fs->currentsector.sector[36] = (uint8_t)((fs->fat_sectors >> 0x00) & 0xFF);
		fs->currentsector.sector[37] = (uint8_t)((fs->fat_sectors >> 0x08) & 0xFF);
		fs->currentsector.sector[38] = (uint8_t)((fs->fat_sectors >> 0x10) & 0xFF);
		fs->currentsector.sector[39] = (uint8_t)((fs->fat_sectors >> 0x18) & 0xFF);
		...
		// BPB_RootClus
		fs->currentsector.sector[44] = (uint8_t)((fs->rootdir_first_cluster >> 0x00) & 0xFF);
		fs->currentsector.sector[45] = (uint8_t)((fs->rootdir_first_cluster >> 0x08) & 0xFF);
		fs->currentsector.sector[46] = (uint8_t)((fs->rootdir_first_cluster >> 0x10) & 0xFF);
		fs->currentsector.sector[47] = (uint8_t)((fs->rootdir_first_cluster >> 0x18) & 0xFF);

		// BPB_FSInfo
		fs->currentsector.sector[48] = (uint8_t)((fs->fs_info_sector >> 0) & 0xFF);
		fs->currentsector.sector[49] = (uint8_t)((fs->fs_info_sector >> 8) & 0xFF);
	...
	if (fs->disk.write(fs->disk.data, boot_sector_lba, fs->currentsector.sector, 1))
		return 1;
	else
		return 0;
}
...
static int fatfs_create_fsinfo_sector(struct fatfs *fs, uint32 sector_lba)
{
    ...
    if (fs->disk.write(fs->disk.data, sector_lba, fs->currentsector.sector, 1))
        return 1;
    else
        return 0;
}
...
static int fatfs_erase_fat(struct fatfs *fs, int is_fat32)
{
    ...
    if (!fs->disk.write(fs->disk.data, fs->fat_begin_lba + 0, fs->currentsector.sector, 1))
        return 0;

    // Zero remaining FAT sectors
    memset(fs->currentsector.sector, 0, FAT_SECTOR_SIZE);
    for (i=1;i<fs->fat_sectors*fs->num_of_fats;i++)
        if (!fs->disk.write(fs->disk.data, fs->fat_begin_lba + i, fs->currentsector.sector, 1))
            return 0;
