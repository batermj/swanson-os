...
int fatfs_init(struct fatfs *fs)
{
    ...
    if (!fs->disk.read(fs->disk.data, 0, fs->currentsector.sector, 1))
        return FAT_INIT_MEDIA_ACCESS_ERROR;
    ...
    if (!fs->disk.read(fs->disk.data, fs->lba_begin, fs->currentsector.sector, 1))
        return FAT_INIT_MEDIA_ACCESS_ERROR;
...
int fatfs_sector_reader(struct fatfs *fs, uint32 start_cluster, uint32 offset, uint8 *target)
{
    ...
    else if (lba != fs->currentsector.address)
    {
        ...
        return fs->disk.read(fs->disk.data, fs->currentsector.address, fs->currentsector.sector, 1);
    }
    else
        return 1;
}
